...
const Notifications = () => {
    ...
    const getNotificationMessage = (notification) => {
        ...
        switch (code) {
            case "NEW_DOCUMENT":
                return t("notifications.payloads.newDocument", { title });
            case "NEEDS_APPROVAL":
                return t("notifications.payloads.needsApproval", { title });
            case "DOCUMENT_APPROVED":
            ...
            case "DOCUMENT_COMMENTED":
                return t("notifications.payloads.documentCommented", { title });
            default:
                return notification.payload || t("notifications.activityAlert");
        }
    };

    return (
        <div
            className={`min-h-screen transition-colors duration-300 ${isDarkMode ? "bg-slate-900 text-white" : "bg-gray-50 text-gray-900"}`}
        >
            <div className="max-w-4xl mx-auto py-12 px-4">
                <div className="mb-8 flex items-center justify-between">
                    <div>
                        <Link
                            to="/dashboard"
                            className="flex items-center text-sm font-bold text-purple-600 hover:text-purple-400 mb-2 transition-colors"
                        >
                        ...
                        <p
                            className={`mt-2 font-medium ${isDarkMode ? "text-slate-400" : "text-gray-500"}`}
                        >
                ...
                {loading ? (
                    ...
                    <div
                        className={`text-center py-20 rounded-3xl border-2 border-dashed ${isDarkMode ? "bg-slate-800/30 border-slate-700 text-slate-500" : "bg-white border-gray-200 text-gray-400"}`}
                    >
                            ...
                            <div
                                ...
                                className={`group p-6 rounded-3xl border transition-all duration-300 shadow-sm relative overflow-hidden ${
                                    n.is_read
                                        ...
                                        : isDarkMode
                                          ? "bg-slate-800 border-purple-500/30 shadow-purple-500/5"
                                          : "bg-white border-purple-200 shadow-purple-600/5"
                                }`}
                            >
                                {!n.is_read && (
                                ...
                                <div className="flex gap-5">
                                    <div
                                        className={`h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 ${isDarkMode ? "bg-slate-900 border border-slate-700" : "bg-gray-50 border border-gray-100"}`}
                                    >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-1">
                                            <h3
                                                className={`text-lg font-black ${isDarkMode ? (n.is_read ? "text-slate-400" : "text-white") : n.is_read ? "text-gray-500" : "text-gray-900"}`}
                                            >
                                            ...
                                            >
                                                {n.is_read
                                                    ...
                                                    : t(
                                                          "notifications.newActivity",
                                                      )}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <p
                                                className={`text-sm font-bold ${isDarkMode ? "text-purple-400" : "text-purple-600"}`}
                                            >
                                                {n.document?.title ||
                                                    t(
