...
const NotificationBell = () => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div
                    className={`absolute right-0 mt-3 w-80 rounded-2xl shadow-2xl overflow-hidden border transition-all z-50 transform origin-top-right ${
                        isDarkMode
                ...
                >
                    <div
                        className={`px-4 py-3 border-b flex justify-between items-center ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-gray-50 border-gray-100"}`}
                    >
                        <span
                            className={`text-sm font-black uppercase tracking-wider ${isDarkMode ? "text-white" : "text-gray-900"}`}
                        >
                        ...
                        <Link
                            ...
                            className="text-xs font-bold text-purple-600 hover:text-purple-400 flex items-center"
                        >
                    ...
                    <div className="max-h-96 overflow-y-auto scrollbar-hide">
                        {recentNotifications.length > 0 ? (
                            recentNotifications.map((n) => (
                                <div
                                    ...
                                    className={`px-4 py-4 border-b flex gap-3 cursor-pointer transition-colors ${
                                        n.is_read
                                            ? isDarkMode
                                                ? "hover:bg-slate-700/50 grayscale opacity-60"
                                ...
                                >
                                    <div
                                        className={`mt-1 h-2 w-2 rounded-full shrink-0 ${n.is_read ? "bg-transparent" : "bg-purple-600 shadow-sm"}`}
                                    />
                                    <div className="flex-1 min-w-0">
                                        <p
                                            className={`text-xs font-black uppercase tracking-tighter mb-1 ${isDarkMode ? "text-slate-400" : "text-gray-500"}`}
                                        >
                                            {n.document?.title ||
                            ...
                            <div
                                className={`px-4 py-10 text-center text-sm font-bold ${isDarkMode ? "text-slate-500" : "text-gray-400"}`}
                            >
